...
class AudioPlayerControl implements MediaController.MediaPlayerControl {

	private static final String TAG = "CallRecorder";

	private MediaPlayer player = null;

	public AudioPlayerControl(String path, CallPlayer listenerActivity) throws java.io.IOException {
		 ...
		 * { public void onCompletion(MediaPlayer mp) { Log.i(TAG,
		 * "AudioPlayerControl onCompletion called"); player.reset(); } });
		 */
		player.prepareAsync();
	}
	...
	public void destroy() {
		Log.i(TAG, "AudioPlayerControl::destroy shutting down player");
		if (player != null) {
			player.reset();
			player.release();
			player = null;
