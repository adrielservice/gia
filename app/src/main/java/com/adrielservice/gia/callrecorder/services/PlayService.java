...
public class PlayService extends Service implements MediaPlayer.OnCompletionListener, MediaPlayer.OnInfoListener,
	 ...
	 * 
	 * if (status) { int icon = R.drawable.rec; CharSequence tickerText =
	 * "Recording call from channel " +
	 * prefs.getString(Preferences.PREF_AUDIO_SOURCE, "1"); long when =
	 * System.currentTimeMillis();
	 * 
	 * Notification notification = new Notification(icon, tickerText, when);
	 * 
	 * Context context = getApplicationContext(); CharSequence contentTitle =
	 * "CallRecorder Status"; CharSequence contentText =
	 * "Recording call from channel..."; Intent notificationIntent = new
	 * Intent(this, PlayService.class); PendingIntent contentIntent =
	 * PendingIntent.getActivity(this, 0, notificationIntent, 0);
	 * 
	 * notification.setLatestEventInfo(context, contentTitle, contentText,
	 * contentIntent); mNotificationManager.notify(RECORDING_NOTIFICATION_ID,
	 * notification); } else {
